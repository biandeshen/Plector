...
# 图片处理模块 v2.5 - 性能与安全优化版（Claude Code 可执行版）

> 修复内容：
>   P0 DNS 解析缓存 - 添加缓存 + TTL + 容量限制
>   P0 重定向信息泄露 - 仅显示域名
>   P0 IPv6 支持 - 完整的私有地址检查
>
> 共 1 步，直接替换文件。
...
def register_backend(
    ...
    参数:
        ...
        server: MCP Server 名称（type=m后端必须指定 skill")
...
for ip, expected in ipv6_tests:
    ...
    print(f'  {status} {ip}: is_private={result} (expected={expected})')

# 测试 DNS 缓存
print(f'\nDNS 缓存统计: {get_dns_cache_stats()}')

# 测试正常图片
ok, msg = validate_image_source('https://httpbin.org/image/png')
print(f'\n正常图片: ok={ok}, msg={msg[:60] if msg else \"\"}')

# 测试缓存效果
print(f'DNS 缓存统计: {get_dns_cache_stats()}')
"
```
...
git commit -m "fix(core): 图片处理模块性能与安全优化（DNS缓存/IPv6/信息脱敏）"
git push
```
...
| 优先级 | 问题 | 修复方式 |
|--------|------|---------|
| P0 | DNS 解析缓存缺失 | 新增 `_DNSCache` 类（TTL 5分钟，最大 1000 条） |
| P0 | 重定向信息泄露 | 仅显示重定向目标域名，不显示完整路径 |
| P0 | IPv6 支持不完整 | `_is_private_ip` 已使用 `ipaddress` 模块，自动支持 IPv6 |

## v2.4 → v2.5 变化

| 项目 | v2.4 | v2.5 |
|------|------|------|
| DNS 缓存 | 无 | `_DNSCache` 类（TTL + 容量限制） |
| 重定向错误信息 | 显示完整 URL | 仅显示域名 |
| IPv6 检查 | 基础支持 | 完整支持（is_private/is_loopback/is_link_local/is_reserved/is_multicast） |
| 错误信息 | 包含具体路径/URL | 脱敏处理，不泄露敏感信息 |
...
URL 输入
  ↓
1. urlparse 解析（检查协议/主机名）
  ↓
2. 主机名是否是 IP？
  ├─ 是 → ipaddress 检查（IPv4 + IPv6）
  └─ 否 → DNS 缓存检查
           ├─ 命中 → 使用缓存的 IP
           └─ 未命中 → DNS 解析 → 缓存结果
  ↓
...
| 函数 | 用途 |
|------|------|
| `clear_dns_cache()` | 清空 DNS 缓存 |
| `get_dns_cache_stats()` | 获取缓存统计（size/max_size/ttl） |
