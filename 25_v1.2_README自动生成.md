...
# Plector README 自动生成 + 自动构建（Claude Code 可执行版）

> 本文档为增量更新，v1.1.0 已标记，WebSocket 渠道已完成。
> 本次更新：
> 1. 创建 README 自动生成脚本
> 2. 生成 README.md
> 3. pre-commit 钩子自动更新
> 4. GitHub Actions 自动更新
>
> 共 5 步，按顺序执行。
...
```bash
python -m py_compile scripts/generate_readme.py
```

### 提交

```bash
git add scripts/generate_readme.py
git commit -m "feat(scripts): 添加 README 自动生成脚本"
```
...
**预期**：看到完整的 README 内容，包含：
- 快速开始
- 核心能力
- 项目结构
- 技能清单（6 个技能 + 1 个 MCP Server）
- 标准对齐
- Harness
- 渠道
- 文档
...
```bash
git add README.md
git commit -m "docs: 生成 README.md（6 skills, 21 tools）"
```

---

## 第三步：pre-commit 钩子自动更新

修改 `.pre-commit-config.yaml`，在末尾添加：
  ...
  - repo: local
    hooks:
      - id: generate-readme
        ...
        language: system
        files: ^(skills/.*/skill\.json|servers/.*\.py)$
...
```bash
# 测试钩子
pre-commit run generate-readme --all-files
```

**预期**：
```
Generate README.md................................Passed
```
...
git commit -m "chore: pre-commit 添加 README 自动更新钩子"
```

---

## 第四步：GitHub Actions 自动更新

创建 `.github/workflows/readme.yml`：
    ...
    steps:
      ...
      - name: Commit README
        run: |
          ...
          git add README.md
          git diff --staged --quiet || git commit -m "docs: auto-update README.md"
          git push
```
...
```bash
# 检查 YAML 语法
python -c "import yaml; yaml.safe_load(open('.github/workflows/readme.yml')); print('YAML OK')"
```
...
git commit -m "ci: GitHub Actions 自动更新 README"
```
...
**预期**：
```
README.md 已生成 (XXXX 字符)
输出: E:\产品\Plector\README.md
```
...
with open('README.md', encoding='utf-8') as f:
    content = f.read()
assert '# Plector' in content
assert 'quick' not in content.lower() or '快速开始' in content
assert 'health_monitor' in content
assert 'code_writer' in content
assert 'file_utils' in content
...
```bash
git add -A
git commit -m "feat: README 自动生成 + pre-commit 钩子 + GitHub Actions"
git push
```
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `scripts/generate_readme.py` | 新建 | README 自动生成脚本 |
| `README.md` | 新建 | 自动生成的项目文档 |
| `.pre-commit-config.yaml` | 修改 | 添加 README 自动更新钩子 |
| `.github/workflows/readme.yml` | 新建 | GitHub Actions 自动更新 |
...
# 自动更新（pre-commit）
git add skills/new_skill/skill.json
git commit -m "feat(new_skill): 添加新技能"
# → pre-commit 自动运行 generate_readme.py
# → README.md 自动更新
# → 一起提交

# 自动更新（GitHub Actions）
git push
# → Actions 检测到 skill.json 变化
# → 自动运行 generate_readme.py
# → 自动提交 README.md
```
