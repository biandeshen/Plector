...
class EventBusV2:
    ...
    def subscribe(
        self, event_type: str, handler: Callable, filter_func: Callable | None = None, use_weak: bool | None = None
    ):
        """
        注册事件处理器
    ...
    async def publish(
        self, event_type: str, data: dict, source: str = "plector", track_history: bool = True
    ) -> list[dict]:
        """
        ...
        """
        event = Event(id=f"{source}-{int(time.time() * 1000)}", source=source, type=event_type, data=data)
...
# 全局实例
_instance: EventBusV2 | None = None
