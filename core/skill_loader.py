...
class SkillLoader:
    ...
    async def warmup(self, skill_names: list[str]) -> dict[str, bool]:
        ...
        return {name: not isinstance(r, Exception) for name, r in zip(skill_names, results, strict=False)}
    ...
    async def _discover_all(self):
        ...
        for item in self.base_path.iterdir():
            if item.is_dir() and (item / "skill.json").exists() and item.name not in self._cache:
    ...
    def get_loaded_modules(self) -> list[str]:
        """获取已加载的模块列表"""
        return [name for name, info in self._cache.items() if info.is_loaded and info.module]
