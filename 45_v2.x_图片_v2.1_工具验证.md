...
# 图片处理模块 v2.1 - 工具验证版（Claude Code 可执行版）

> 改进思路：
>   本地文件：检查文件属性（存在/格式/大小）
>   网络图片：HEAD 请求验证（可达/Content-Type）
>
> 共 1 步，直接替换文件。
...
def validate_image_source(image_source: str) -> Tuple[bool, str]:
    ...
    """
    if        return _validate_url(image_source)
    else:
...
# 测试不存在的文件
ok, msg = validate_image_source('./not_exist.png')
print(f'不存在: {ok}, {msg}')

# 测试 URL
ok, msg = validate_image_source('https://www.python.org/static/community_logos/python-logo.png')
print(f'URL: {ok}, {msg}')
"
```
...
git commit -m "refactor(core): 图片处理模块改用工具验证（移除白名单）"
```
...
| 方式 | 旧版（白名单） | 新版（工具验证） |
|------|---------------|-----------------|
| 本地文件 | 检查目录是否在白名单 | 检查文件属性（存在/格式/大小） |
| 网络图片 | 检查域名是否在白名单 | HEAD 请求验证（可达/类型） |
| 新增目录 | 需要改代码 | 自动支持 |
| 新增域名 | 需要改代码 | 自动支持 |
| 灵活性 | 低 | 高 |
