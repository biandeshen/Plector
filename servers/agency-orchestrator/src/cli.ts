...
async function handleCompose(): Promise<void> {
  ...
  if (!description) {
    ...
    console.error('  --provider <name>   LLM 提供商 (默认 claude-code，免 API key)');
    console.error('  --model <name>      模型名 (默认 claude-sonnet-4-20250514)');
    process.exit(1);
  }

  const provider = (getArgValue('--provider') || 'claude-code') as LLMConfig['provider'];
  const model = getArgValue('--model') || (provider === 'deepseek' ? 'deepseek-chat' : provider === 'ollama' ? 'llama3' : 'claude-sonnet-4-20250514');
