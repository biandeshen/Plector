...
| 阶段 | 方式 | 后端改动 | 耗时预估 |
|------|------|---------|---------|
| Phase 1 | Vue 3 CDN 单文件 | 零改动 | - |
| Phase 2 | Vite + Vue 3 SPA | 添加 StaticFiles | - |
...
│   ├── NewChatButton.vue
│   ├── SearchInput.vue              (NEW: 对话搜索)
│   ├── ConversationGroup.vue        (NEW: 按日期分组)
│   │   └── ConversationItem.vue     (标题、时间、重命名、删除)
│   └── SidebarToggle.vue            (NEW: 移动端折叠)
│
├── ChatMain.vue
...
└── actions
    ...
    └── stopGeneration()         ← NEW: 关闭 WebSocket 连接
...
### Phase 1: Vue 3 CDN 即插即用

**目标**: 快速获得响应式架构收益，零后端改动

**步骤**:
1. 创建 `chat_vue.html`，引入 Vue 3 CDN
2. 将全局状态迁移为 `ref()`/`reactive()`
3. 将 DOM 操作迁移为 Vue 模板 (`v-for`, `v-if`, `v-show`)
4. 实现 `useWebSocket` 组合函数
5. 添加 CSS 过渡动画 (工具面板展开/折叠)
6. 添加停止生成按钮
7. 添加代码块语言标签+独立复制
8. 添加移动端响应式基础
9. 测试所有 WebSocket 事件流
10. 替换 `chat.html`

**交付物**: 单个 `chat.html` 文件，功能等价+增强

### Phase 2: Vite + Vue 3 SPA 工程化

**目标**: 完整组件化，TypeScript 类型安全，开发体验优化

**步骤**:
1. `npm create vite@latest frontend -- --template vue-ts`
2. 将 CDN 版本的组合函数拆分为 `.ts` 文件
3. 将模板拆分为 `.vue` SFC 组件
4. 引入 Pinia 状态管理
5. 添加 Vitest 单元测试
6. 配置 Vite 代理 (`/ws` → FastAPI, `/api/*` → FastAPI)
7. `websocket.py` 添加 `StaticFiles` 挂载
8. 实现虚拟滚动 (长对话)
9. 实现对话搜索、日期分组
10. 实现亮色主题
11. 实现 Markdown 增量渲染优化

**交付物**: `frontend/` 目录，`npm run build` → `dist/`
