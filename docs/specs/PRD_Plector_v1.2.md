...
| **Should** | Chat SPA（Vue 3） | 现代化 Web 对话界面，流式渲染、工具调用可视化 |
...
**故事6（用户）：通过 Web 界面与 Agent 对话**
> 作为用户，我希望通过浏览器访问 Chat 界面，与 Agent 实时对话，并能看到工具调用过程和思考过程。

**验收标准**：
- 访问 `/chat` 即可使用 Vue 3 SPA 界面。
- 支持 WebSocket 流式渲染，逐字显示 Agent 回复。
- 工具调用以折叠面板展示，显示工具名称、状态、耗时、参数和结果。
- 摘要行显示工具调用数和过程消息数（如 "5 个工具调用，3 条过程消息"）。
- 面板默认折叠，点击展开查看详情。
- 支持停止生成、复制消息、代码块语言标签和独立复制按钮。
- 侧边栏支持对话搜索、新建、重命名、删除。
- 支持亮色/暗色主题切换。
- 移动端响应式布局。
