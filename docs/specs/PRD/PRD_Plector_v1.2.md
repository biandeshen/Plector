...
### 10.4 动态增强机制（v2.1.1 新增）

> **详细实现方案**：参见 [65_记忆系统增强 - 动态记忆生态系统 v2.1.1.md](../../研究文档/65_记忆系统增强%20-%20动态记忆生态系统%20v2.1.1.md)

**核心增强机制**：

| 机制 | 说明 | 效果 |
|------|------|------|
| **检索即复习** | 每次搜索命中且相关度 ≥0.4 的记忆自动静默加固 | 模拟检索练习效应 |
| **稳定性自增长** | 每次加固时 stability × 1.08，上限 100.0 | 高频记忆长期保持鲜活 |
| **存在时长微调** | 500 轮后稳定性 ×1.2，模拟"老记忆更顽固" | 老记忆衰减减缓 |
| **批量防抖** | 单次搜索最多加固 20 条，先返回再执行 | 不污染搜索结果 |

**轮次计数器**：
- 新增 `core/turn_counter.py` 持久化轮次（存储在 `.plector_turn`）
- 保证轮次跨会话累积，对 AI 来说"时间 = 对话轮次"

**新增文件清单**：
| 文件 | 改动 |
|------|------|
| `core/turn_counter.py` | **新增**：持久化轮次计数器 |
| `core/vector_memory.py` | **修改**：集成衰减、多路召回、检索即复习、稳定性自增长 |
| `skills/memory/implementation.py` | **修改**：新参数和工具定义 |
| `skills/memory/skill.json` | **替换**：v2.1.1 工具清单 |
| `skills/memory/SKILL.md` | **替换**：v2.1.1 文档 |

**性能注意事项**：

| 操作 | 数据量 | 性能 |
|------|--------|------|
| 语义/触景/触类检索 | 任意 | ✅ ChromaDB 索引，O(log n) |
| 时序联想 (`_search_by_temporal`) | < 500 条 | ✅ 全量遍历可接受 |
| 时序联想 | > 1000 条 | ⚠️ 建议改为 ChromaDB 时间范围查询 |
| 温故知新 | 任意 | ✅ 仅每 50 轮触发，限 top-3 |
...
**文档状态**: 已定稿
**版本历史**：
- v1.6：五角色 Agent 审查结论整合 + 记忆系统 v2.1.1 详细方案引用（检索即复习、稳定性自增长、存在时长微调、批量防抖）
