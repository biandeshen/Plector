...
# 图片处理模块 v2.4 - SSRF 防护版（Claude Code 可执行版）

> 修复内容：
>   P0 SSRF 重定向绕过 - 禁止重定向
>   P0 DNS Rebinding - 请求时重新验证 IP
>   P1 流式检查日志
>   P2 httpx 顶层导入 + 魔法数字常量化
>
> 共 1 步，直接替换文件。
...
# 测试重定向防护
ok, msg = validate_image_source('https://httpbin.org/redirect-to?url=https://httpbin.org/image/png')
print(f'重定向: ok={ok}')
if not ok:
    print(f'  msg: {msg[:80]}')
...
print(f'正常图片: ok={ok}, msg={msg[:60]}')

# 测试本地文件
ok, msg = validate_image_source('./config.yaml')
print(f'本地文件: ok={ok}, msg={msg[:60]}')
"
```
...
git commit -m "fix(core): 修复 SSRF 重定向绕过和 DNS Rebinding 漏洞"
git push
```
...
| 优先级 | 问题 | 修复方式 |
|--------|------|---------|
| P0 | SSRF 重定向绕过 | `follow_redirects=False` + 检查重定向状态码 |
| P0 | DNS Rebinding | 新增 `_resolve_and_check_ip()` 函数，请求前解析并检查 |
| P1 | 流式检查日志 | 添加 `logger.debug` 输出下载字节数 |
| P2 | httpx 导入 | 延迟导入 + 模块级缓存 |
| P2 | 魔法数字 | 定义 `STREAM_CHUNK_SIZE = 8192` 常量 |

## v2.3 → v2.4 变化

| 项目 | v2.3 | v2.4 |
|------|------|------|
| 重定向处理 | `follow_redirects=True` | `follow_redirects=False` + 拒绝重定向 |
| DNS 防护 | 仅检查主机名 | 解析 IP 后检查（防 Rebinding） |
| httpx 导入 | 函数内延迟导入 | 模块级 `_get_httpx()` 缓存 |
| 块大小 | 硬编码 8192 | 常量 `STREAM_CHUNK_SIZE` |
