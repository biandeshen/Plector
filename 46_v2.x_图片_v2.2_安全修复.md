...
# 图片处理模块 v2.2 - 安全修复版（Claude Code 可执行版）

> 修复内容：
>   🔴 路径遍历漏洞 - 限制根目录
>   🔴 动态键名不安全 - 固定键名结构
>   🔴 URL 误判边界 - 严格正则匹配
>   🔴 Content-Length 解析 - 添加异常处理
>   🔴 输入验证缺失 - 类型和空值检查
>   🟡 SSRF 风险 - 拒绝内网地址
>   🟡 超时优化 - 10秒改为5秒
>   🟡 后端排序 - 按优先级选择
...
```bash
python -c "
from core.image_handler import validate_image_source, parse_image_command

# 测试路径遍历防护
ok, msg = validate_image_source('../../etc/passwd')
print(f'路径遍历: ok={ok}, msg={msg[:50]}')

# 测试绝对路径防护
ok, msg = validate_image_source('/etc/passwd')
print(f'绝对路径: ok={ok}, msg={msg[:50]}')

# 测试输入验证
result = parse_image_command(None)
print(f'None输入: {result}')

result = parse_image_command('')
print(f'空字符串: {result}')

# 测试本地文件
ok, msg = validate_image_source('./config.yaml')
print(f'本地文件: ok={ok}, msg={msg[:50]}')
"
```
...
git commit -m "fix(core): 修复图片处理模块安全问题（路径遍历/SSRF/输入验证）"
git push
```
...
|------|------|---------|
| 🔴 路径遍历漏洞 | ✅ | 禁止绝对路径 + 禁止 `..` |
| 🔴 动态键名不安全 | ✅ | 固定 `server` 和 `skill` 两个键 |
| 🔴 URL 误判边界 | ✅ | 严格正则 + `re.match(r"^https?://")` |
| 🔴 Content-Length 解析 | ✅ | try-except + 负值检查 |
| 🔴 输入验证缺失 | ✅ | isinstance 检查 + 空值检查 |
| 🟡 SSRF 风险 | ✅ | 内网地址检测（ipaddress 模块） |
| 🟡 超时过长 | ✅ | 10秒 → 5秒 |
| 🟡 后端排序 | ✅ | get_best_backend 使用 get_available_backends |
| 🟡 缺少 __all__ | ✅ | 添加导出声明 |
